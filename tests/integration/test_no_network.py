...
"""Integration: compile/validate make zero network calls (Story I.e, AC-9 / SC-2).

A socket-level sandbox monkeypatches `socket.socket.connect` to raise.
Because `compile_exercise` / `validate_exercise` only read local files,
they must succeed with the sandbox active; if either ever opened a
socket the test fails closed.
"""
...
def test_validate_exercise_makes_no_network_calls(
    no_network: None, tmp_base_dir: Path, sample_yaml: Path
) -> None:
    assert validate_exercise(sample_yaml, tmp_base_dir) == []
